...
const Notifications = () => {
  const { inboxNotifications } = useInboxNotifications();  //Fetches inbox notifications
  const { count } = useUnreadInboxNotificationsCount();   //Fetches the count of unread notifications.
  ...
  return (
    <Popover>
      ...
      <PopoverContent align="end" className="shad-popover">
        <LiveblocksUIConfig  //Provides customization for notification types, such as mentions 
          // you use it here to override the default text for a mention notification
          overrides={{
            INBOX_NOTIFICATION_TEXT_MENTION: (user: ReactNode) => (
              <>{user} mentioned you.</> //to show a custom message (e.g- "User mentioned you")
            )
          }}
        >
                ...
                kinds={{
                  thread: (props) => (
                    <InboxNotification.Thread {...props}
                      showActions={false}   //hides action buttons for notification like "Mark as Read"
                      showRoomName={false}
                    />
                  ),
                  textMention: (props) => (
                  ...
                  $documentAccess: (props) => (  //Renders a custom notification for document access using
